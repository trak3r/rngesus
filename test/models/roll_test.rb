...
class RollTest < ActiveSupport::TestCase
  ...
  test 'should accept valid dice formats' do
    valid_formats = ['D6', '2D6', 'd20', '3d8', 'D100']
    
    valid_formats.each do |dice_format|
      roll = Roll.new(name: 'Test Roll', dice: dice_format, randomizer: randomizers(:encounter))
      assert roll.valid?, "#{dice_format} should be valid but got errors: #{roll.errors.full_messages}"
    end
  end
  ...
  test 'outcome returns rolled value and matching result' do
    ...
    if result
      assert_includes ['Close', 'Medium', 'Far'], result.name
      assert result.value <= rolled, "Result value #{result.value} should be <= rolled #{rolled}"
    end
  end
